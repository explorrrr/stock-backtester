    ...
    <R as Repositories>::PriceTimeSeriesRepo: Sync,
{
    ...
    pub async fn backtest(&self, request: BackTestRequest) -> BackTestResult {
        ...
        let time_series_entity = self.price_time_series_repository.get_time_series_by_ticker(&request.ticker).await;

        let start_time = Utc.datetime_from_str(&request.orders[0].time_stamp, "%Y/%m/%d %H:%M:%S").unwrap();
        ...
        for order in request.orders {
            ...
            let order_datetime = Utc.datetime_from_str(&order.time_stamp, "%Y/%m/%d %H:%M:%S").unwrap();
